...
@attendance_per_month_bp.route(f'/student_group_dates/', methods=['GET', 'POST'])
@jwt_required()
def student_group_dates():
    identity = get_jwt_identity()
    user = User.query.filter(User.classroom_user_id == identity).first()

    if request.method == "GET":
        response = requests.get(f'{gennis_server_url}/api/group_classroom_attendance/student_group_dates2_classroom/{user.platform_id}/')
        return jsonify(response.json())


@attendance_per_month_bp.route(f'/combined_attendances2/', methods=['GET', 'POST'])
@jwt_required()
def combined_attendances2():
    identity = get_jwt_identity()
    user = User.query.filter(User.classroom_user_id == identity).first()

    if request.method == "GET":
        response = requests.get(
            f'{gennis_server_url}/api/group_classroom_attendance/combined_attendances_classroom/{user.platform_id}/')
        return jsonify(response.json())
    else:
